...
class VSRDataset(VQADataset):
    ...
    def process_text(self, ann):
        # TODO remove this
        instruction = ann["question"][:-2] + ann["question"][-1]
        answer = ann["anwser"]
        src_text = f"<image> <image> User: {instruction} GPT:<answer> {answer} <|endofchunk|>"
...
if __name__ == "__main__":
    ...
    dataset = VSRDataset(
        ...
        ann_paths=
        ["/home/scratch.chaoweix_nvresearch/visual_instruction/vlm/workspace/datasets/visual-spatial-reasoning/vsr_data_1.jsonl",
         ]
    )
    ...
    for batch in train_dataloader:
        ...
        plt.imshow(image)
        plt.savefig("/home/scratch.chaoweix_nvresearch/visual_instruction/vlm/examples/vsr1.png")

        plt.figure()
        image = images.squeeze(2)[1][1].permute(1, 2, 0).numpy()
        plt.imshow(image)
        plt.savefig("/home/scratch.chaoweix_nvresearch/visual_instruction/vlm/examples/vsr2.png")
