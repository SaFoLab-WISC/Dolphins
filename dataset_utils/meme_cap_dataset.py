...
class MEMECAPDataset(VQADataset):
    def process_image(self, ann):
        image_path = os.path.join(self.vis_root, ann["img_fname"])
    ...
    def process_text(self, ann):
        # TODO remove this
        meme_captions = " ".join(ann["meme_captions"])
        instruction = random.choice(QUESTIONS)
        conversation = self.prompter(question=[instruction], response=[meme_captions], num_images=1, num_convs=1)
...
if __name__ == "__main__":
    ...
    dataset = MEMECAPDataset(
        ...
        vis_root="/home/scratch.chaoweix_nvresearch/visual_instruction/vlm/workspace/datasets/MEME_Cap/memes",
        ann_paths=
        ["/home/scratch.chaoweix_nvresearch/visual_instruction/vlm/workspace/datasets/MEME_Cap/memes-trainval.json",
         ]
    )
    ...
    for batch in train_dataloader:
        ...
        plt.imshow(image)
        plt.savefig("/home/scratch.chaoweix_nvresearch/visual_instruction/vlm/examples/meme.png")
