...
class SVITBboxDataset(VQADataset):
    ...
    def process_image(self, ann):
        ...
        try:
            image_path = os.path.join(self.vis_root, "VG_100K", image_path.split("/")[-1].strip(" "))
            image = Image.open(image_path).convert("RGB")
        except:
            image_path = os.path.join(self.vis_root, "VG_100K_2", image_path.split("/")[-1].strip(" "))
            image = Image.open(image_path).convert("RGB")
