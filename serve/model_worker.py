...
import transformers
from peft import get_peft_model, LoraConfig, get_peft_model_state_dict, PeftConfig, PeftModel
...
class ModelWorker:
    ...
    def load_model(self, lm_path, checkpoint_path, num_gpus, load_in_8bit, load_pt):
        ...
        elif self.model_name == "otter":
            model =  OtterForConditionalGeneration.from_pretrained("./models/otter-video-llama", torch_dtype=torch.float16, device_map="auto")
            if self.use_lora:
                lora_config = LoraConfig(**otter_tuning_config)
                model.config.update({"lora_config": otter_tuning_config})
        ...
        context_len = 2048
        image_processor = DefaultTransform(image_size=336)
    ...
    def generate_stream(self, params):
        ...
        if images is not None:
            ...
            if len(images) > 0:
                ...
                if is_video is True:
                    vision_x = torch.stack([image_processor(image) for image in images], dim=0).unsqueeze(0).unsqueeze(0)
                    assert vision_x.shape[2] == len(
                        images)  # dim of vision_x: [B, T, F, C, H, W], make sure conditioned on frames of the same video
                else:
                    vision_x = torch.stack([image_processor(images)], dim=0).unsqueeze(1).unsqueeze(0)
                vision_x = vision_x.to(self.device, dtype=torch.float16)
        ...
        #         image.save(f"/home/yingzi/Visual-Instruction-Meta-Learning/serve/examples/examples/BDD-X_{i}_{j}.png")
        ...
        generated_text = generated_text[generated_text.rfind(DEFAULT_ANSWER_TOKEN) + len(DEFAULT_ANSWER_TOKEN): \
                generated_text.rfind(DEFAULT_DEMO_END_TOKEN)]
